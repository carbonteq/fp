...
describe("Option.asyncGenAdapter", () => {
  ...
  describe("sync Option with async map/flatMap", () => {
    it("should handle explicit async methods via adapter", async () => {
      ...
      const result = await Option.asyncGenAdapter(async function* ($) {
        const value = yield* $(
          await Option.Some(3).map(asyncDouble).toPromise(),
        )
        return value
      })
    ...
    it("should handle flatMap via adapter", async () => {
      ...
      const result = await Option.asyncGenAdapter(async function* ($) {
        const value = yield* $(
          await Option.Some(5).flatMap(asyncValidate).toPromise(),
        )
        return value
      })
    ...
    it("should chain multiple sync Options with async operations", async () => {
      ...
      const result = await Option.asyncGenAdapter(async function* ($) {
        ...
        const transformed = yield* $(
          await Option.Some(first).map(asyncTransform).toPromise(),
        )
    ...
    it("should handle filter with async predicate", async () => {
      ...
      const result = await Option.asyncGenAdapter(async function* ($) {
        const value = yield* $(
          await Option.Some(4).filter(asyncIsEven).toPromise(),
        )
        return value * 2
      })
    ...
    it("should short-circuit when async filter returns false", async () => {
      ...
      const result = await Option.asyncGenAdapter(async function* ($) {
        const filtered = await Option.Some(3).filter(asyncIsEven).toPromise()
    ...
    it("should handle zip with async function", async () => {
      ...
      const result = await Option.asyncGenAdapter(async function* ($) {
        const [num, str] = yield* $(
          await Option.Some(42).zip(asyncFetch).toPromise(),
        )
        return { num, str }
      })

      expectTypeOf(result).toEqualTypeOf<Option<{ num: number; str: string }>>()
      expect(result.isSome()).toBe(true)
  ...
  describe("edge cases", () => {
    ...
    it("should propagate promise rejections", async () => {
      ...
      await Option.asyncGenAdapter(async function* ($) {
        ...
        () => {
          throw new Error("Expected asyncGenAdapter to reject")
        },
        (error: unknown) => {
  ...
  describe("explicit async patterns", () => {
    ...
    it("should handle map in adapter", async () => {
      const asyncDouble = async (n: number): Promise<number> => {
        await delay(5)
        return n * 2
      }
      ...
      const result = await Option.asyncGenAdapter(async function* ($) {
        // map returns Promise<Option<number>>, which we await before yielding
        const value = yield* $(
          await Option.Some(3).map(asyncDouble).toPromise(),
        )
    ...
    it("should work with flatMap in adapter", async () => {
      ...
      const result = await Option.asyncGenAdapter(async function* ($) {
        // flatMap returns Promise<Option<number>>, which we await before yielding
        const value = yield* $(
          await Option.Some(5).flatMap(asyncFlatMap).toPromise(),
        )
        expectTypeOf(value).toEqualTypeOf<number>()
        return value
      })
    ...
    it("should combine multiple async operations", async () => {
      ...
      const result = await Option.asyncGenAdapter(async function* ($) {
        ...
        // Another async operation via map
        const b = yield* $(
          await Option.Some(5)
            .map(async (n) => n * 2)
            .toPromise(),
