...
describe("Phase 2 & 3 - Combinators & Helpers", () => {
  describe("map", () => {
    ...
    it("should propagate Err results without calling mapper", () => {
      const result = HybridResult.Err<string, number>("error")
        .map(x => x * 2);
    ...
    it("should handle async Err results with map", async () => {
      const result = HybridResult.Err(Promise.resolve("async error"))
        .map(x => x * 2);
  ...
  describe("flatMap", () => {
    ...
    it("should handle Err results in flatMap", () => {
      const result = HybridResult.Ok(42)
        .flatMap(() => HybridResult.Err("flatMap error"));
    ...
    it("should promote to async with async mappers", async () => {
      const result = HybridResult.Ok(42)
        .flatMap(x => Promise.resolve(HybridResult.Ok(x * 2)));
    ...
    it("should propagate initial Err results", () => {
      const result = HybridResult.Err("initial error")
        .flatMap(x => HybridResult.Ok(x * 2));
  ...
  describe("zip", () => {
    ...
    it("should handle Err results in zip", () => {
      const result = HybridResult.Err<number, string>("error")
        .zip(x => x * 2);
  ...
  describe("flatZip", () => {
    ...
    it("should handle Err results in flatZip", () => {
      const result = HybridResult.Ok(42)
        .flatZip(() => HybridResult.Err("flatZip error"));
    ...
    it("should promote to async with async mappers", async () => {
      const result = HybridResult.Ok(42)
        .flatZip(x => Promise.resolve(HybridResult.Ok(x * 2)));
  ...
  describe("validate", () => {
    ...
    it("should handle async initial result", async () => {
      const result = HybridResult.Ok(Promise.resolve(42))
        .validate([
          x => HybridResult.Ok(x > 0),
          x => HybridResult.Ok(x < 100)
        ]);
  ...
  describe("mapErr", () => {
    ...
    it("should promote to async with async mapper", async () => {
      const result = HybridResult.Err("original error")
        .mapErr(err => Promise.resolve(`Mapped: ${err}`));
  ...
  describe("mapBoth", () => {
    it("should map both Ok and Err values", () => {
      const okResult = HybridResult.Ok(42)
        .mapBoth(x => x * 2, err => `Error: ${err}`);

      expect(okResult.isOk()).toBe(true);
      expect(okResult.unwrap()).toBe(84);

      const errResult = HybridResult.Err("test error")
        .mapBoth(x => x * 2, err => `Error: ${err}`);
    ...
    it("should promote to async with async mappers", async () => {
      const result = HybridResult.Ok(42)
        .mapBoth(x => Promise.resolve(x * 2), err => `Error: ${err}`);
  ...
  describe("orElse", () => {
    ...
    it("should handle async defaults", async () => {
      const syncResult = HybridResult.Err("error")
        .orElse(Promise.resolve(100));
    ...
    it("should handle async results", async () => {
      const asyncResult = HybridResult.Ok(Promise.resolve(42))
        .orElse(0);
  ...
  describe("andThen", () => {
    ...
    it("should handle Err results", () => {
      const result = HybridResult.Ok(42)
        .andThen(() => HybridResult.Err("andThen error"));
  ...
  describe("Complex Integration Tests", () => {
    ...
    it("should handle complex validation scenarios", async () => {
      const result = HybridResult.Ok(Promise.resolve({ age: 25, name: "John" }))
        .validate([
          person => person.age >= 18 ? HybridResult.Ok(true) : HybridResult.Err("too young"),
          person => person.name.length > 0 ? HybridResult.Ok(true) : HybridResult.Err("empty name")
        ])
